...
module durable_tid_v0 #(
  ...
  output logic        trusted_complete_o,
  output logic [63:0] durable_tid_o,
  output logic        busy_o
);

  // Word indices (byte offset >> 2).
  localparam integer W_EPOCH    = 12'h000 >> 2;
  localparam integer W_REQ_LO   = 12'h004 >> 2;
  localparam integer W_PEND_LO  = 12'h008 >> 2;
  localparam integer W_DUR_LO   = 12'h00C >> 2;
  localparam integer W_VIS_LO   = 12'h010 >> 2;
  localparam integer W_FSM      = 12'h014 >> 2;
  localparam integer W_ERROR    = 12'h018 >> 2;
  localparam integer W_PROG     = 12'h01C >> 2;
  localparam integer W_RSTCNT   = 12'h020 >> 2;
  localparam integer W_CTRL     = 12'h024 >> 2;
  localparam integer W_STATUS   = 12'h028 >> 2;
  localparam integer W_DESC0    = 12'h02C >> 2;
  localparam integer W_DESC1    = 12'h030 >> 2;
  localparam integer W_DESC_CRC = 12'h034 >> 2;
  localparam integer W_CRC_CALC = 12'h038 >> 2;
  localparam integer W_TID_LO   = 12'h03C >> 2;
  localparam integer W_TID_HI   = 12'h040 >> 2;
  localparam integer W_REQ_HI   = 12'h044 >> 2;
  localparam integer W_DUR_HI   = 12'h048 >> 2;
  localparam integer W_VIS_HI   = 12'h04C >> 2;
  ...
  localparam logic [31:0] VERSION_VAL = 32'h00000000; // v0
  ...
  localparam logic [2:0] S_SUBMIT   = 3'd1;
  localparam logic [2:0] S_CRC      = 3'd2;
  localparam logic [2:0] S_COMMIT   = 3'd3;
  localparam logic [2:0] S_COMPLETE = 3'd4;

  // ERROR bit positions.
  localparam integer E_CRC    = 0;
  localparam integer E_DUP    = 1;
  localparam integer E_GAP    = 2;
  localparam integer E_MALF   = 3;
  localparam integer E_RSTMID = 4;
  localparam integer E_OVF    = 5;
  ...
  logic [31:0] error;       // sticky, only low 6 bits used
  logic [31:0] progress_cnt;
  logic [31:0] reset_cnt;
  logic [31:0] ctrl;        // only low 3 bits used
  logic        complete_sticky;
  logic        crc_ok_last;
  logic [31:0] desc0, desc1, desc_crc;
  logic [31:0] crc_calc;
  logic [31:0] crc_mid_q;   // registered CRC over descriptor bytes 0..7
  logic [1:0]  commit_hold; // COMMIT latency countdown
  ...
  logic [9:0]  rd_addr_q;
  logic        rd_en_q;
  logic        readdatavalid_q;
  logic        trusted_q;

  // Latched submit image validated in SUBMIT/S_CRC.
  logic [31:0] lat_d0, lat_d1, lat_req, lat_ep, lat_crc;
  logic [63:0] lat_tid;
  ...
  logic [31:0] err_n;
  logic [63:0] req_full;
  logic [31:0] progress_cnt_n, reset_cnt_n;
  logic        complete_sticky_n, crc_ok_last_n;
  logic [31:0] desc0_n, desc1_n, desc_crc_n, crc_calc_n, crc_mid_n;
  logic [1:0]  commit_hold_n;
  logic [31:0] readdata_n, rd_mux;
  logic [9:0]  rd_addr_n;
  logic        rd_en_n, readdatavalid_n, trusted_n;
  logic [31:0] lat_d0_n, lat_d1_n, lat_req_n, lat_ep_n, lat_crc_n;
  logic [63:0] lat_tid_n;
  logic        soft_rst;
  ...
  function [31:0] crc32_blk(input [31:0] crc_in, input [63:0] data);
    reg [31:0] crc;
    integer i, j;
    begin
      crc = crc_in;
  ...
  localparam logic [1:0] COMMIT_HOLD_INIT = COMMIT_LATENCY;
